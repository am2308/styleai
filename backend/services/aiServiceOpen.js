...
// Enhanced AI service with better outfit generation and OpenAI integration
class LocalAIService {
  constructor() {
    console.log('Using enhanced local AI service with OpenAI integration for outfit recommendations');
  }
  ...
  async generateOutfitRecommendations(wardrobeItems, userProfile, occasion = null) {
    try {
      ...
      try {
        const openAIRecommendations = await this.generateOpenAIRecommendations(wardrobeItems, userProfile, occasion);
        ...
        console.error('OpenAI recommendation failed, falling back to local algorithm:', openAIError);
      }
  ...
  async generateOpenAIRecommendations(wardrobeItems, userProfile, occasion) {
    if (!process.env.OPENAI_API_KEY) {
      console.log('OpenAI API key not configured, using local algorithm');
      return null;
    }
    ...
    try {
      ...
      const itemsData = wardrobeItems.map(item => ({
        id: item.id,
        name: item.name,
      ...
      const profileData = {
        ...
        bodyType: userProfile.bodyType || 'Rectangle',
      ...
      const prompt = {
        ...
        messages: [
          {
            role: "system",
            content: `You are a professional fashion stylist AI that creates personalized outfit recommendations. 
            You analyze wardrobe items and user profiles to create stylish, appropriate outfits.
            Your recommendations should consider color coordination, body type, skin tone, style preferences, and occasion appropriateness.
            Provide detailed outfit combinations with confidence scores and styling notes.`
          },
          {
            role: "user",
            content: `Create outfit recommendations based on the following:
            ...
            1. A unique ID
            2. Array of item IDs to include in the outfit
            3. Confidence score (0.0-1.0)
            4. Appropriate occasion
            5. Description of the outfit
            6. Style notes specific to the user's profile
            
            Also provide a wardrobe analysis with:
            1. Strengths of the current wardrobe
            2. Gaps or missing essential items
            3. Suggestions for future additions
            
            Format your response as a valid JSON object with "recommendations" and "wardrobeAnalysis" properties.`
          }
        ],
        temperature: 0.7,
        max_tokens: 2000
      ...
      try {
        ...
        if (jsonMatch) {
          try {
            ...
            console.error('Failed to extract JSON from response:', secondParseError);
            return null;
          }
        } else {
          return null;
      ...
      if (responseJson && responseJson.recommendations && Array.isArray(responseJson.recommendations)) {
        // Add missing items to each outfit
        const enhancedRecommendations = await Promise.all(
          responseJson.recommendations.map(async outfit => {
            const missingItems = await this.identifyMissingItemsWithProducts(outfit, wardrobeItems, userProfile);
            return {
              ...outfit,
              missingItems: missingItems
        ...
        return {
          ...
          wardrobeAnalysis: responseJson.wardrobeAnalysis || this.analyzeWardrobe(wardrobeItems, userProfile)
        };
      }
...
export const generateMarketplaceRecommendations = async (userProfile, filters = {}) => {
  try {
    ...
    // Try to use OpenAI for enhanced recommendations if available
    if (process.env.OPENAI_API_KEY) {
      try {
        const openAIRecommendations = await generateOpenAIMarketplaceRecommendations(userProfile, filters);
        if (openAIRecommendations && openAIRecommendations.length > 0) {
          return openAIRecommendations.slice(0, 20);
        }
      } catch (openAIError) {
        console.error('OpenAI marketplace recommendations failed, falling back to local algorithm:', openAIError);
      }
    }
...
// OpenAI-powered marketplace recommendations
async function generateOpenAIMarketplaceRecommendations(userProfile, filters) {
  try {
    const { category, color, occasion, priceRange } = filters;
    ...
    const prompt = {
      ...
      messages: [
        {
          role: "system",
          content: `You are a fashion expert AI that recommends clothing items based on user profiles and preferences.
          Your recommendations should be specific, detailed, and tailored to the user's style, body type, and skin tone.`
        },
        {
          role: "user",
          content: `Recommend fashion items for a user with the following profile:
          
          USER PROFILE:
          - Skin tone: ${userProfile.skinTone || 'Medium'}
          - Body type: ${userProfile.bodyType || 'Average'}
          - Preferred style: ${userProfile.preferredStyle || 'Casual'}
    ...
    // Transform OpenAI recommendations to match our format
    if (Array.isArray(recommendations)) {
      return recommendations.map((item, index) => ({
        id: `openai_${index}`,
        name: item.name,
        category: item.category || category || 'Tops',
        color: item.color || color || 'Black',
        price: parseFloat(item.price) || 49.99,
        imageUrl: getImageForCategory(item.category || category),
...
export const generateSmartShoppingList = async (wardrobeItems, userProfile, targetOccasions = []) => {
  try {
    // Try to use OpenAI for enhanced shopping list if available
    if (process.env.OPENAI_API_KEY) {
      try {
        const openAIShoppingList = await generateOpenAIShoppingList(wardrobeItems, userProfile, targetOccasions);
        if (openAIShoppingList && openAIShoppingList.length > 0) {
          return { shoppingList: openAIShoppingList };
        }
      } catch (openAIError) {
        console.error('OpenAI shopping list failed, falling back to local algorithm:', openAIError);
      }
    }
...
// OpenAI-powered shopping list
async function generateOpenAIShoppingList(wardrobeItems, userProfile, targetOccasions) {
  try {
    // Prepare wardrobe items data
    const itemsData = wardrobeItems.map(item => ({
      name: item.name,
    ...
    const prompt = {
      ...
      messages: [
        {
          role: "system",
          content: `You are a professional fashion stylist AI that creates personalized shopping recommendations.
          You analyze a user's existing wardrobe and profile to suggest items that would enhance their wardrobe.
          Your recommendations should be specific, practical, and tailored to the user's style, body type, and skin tone.`
        },
        {
          role: "user",
          content: `Create a smart shopping list based on the following:
          ...
          TARGET OCCASIONS: ${targetOccasions.length > 0 ? targetOccasions.join(', ') : 'Any'}
          
          For each recommended item, provide:
          1. Item name
          2. Category (Tops, Bottoms, Dresses, Outerwear, Footwear, Accessories)
          3. Priority (high, medium, low)
          4. Reason for recommendation
          5. Price range (min and max)
    ...
    // Enhance shopping list with marketplace products
      ...
      const enhancedList = await Promise.all(
        shoppingList.map(async (item) => {
          ...
          const searchTerms = [
            ...
            ...(item.occasions || []).map(occ => occ.toLowerCase())
          ].filter(Boolean);
...
function getImageForCategory(category) {
  ...
  return categoryImages[category] || 'https://images.pexels.com/photos/996329/pexels-photo-996329.jpeg?auto=compress&cs=tinysrgb&w=600';
}
